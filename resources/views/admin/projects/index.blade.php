<x-app-layout>
    <x-slot name="header">
        <span class="font-bold">{{ __('Projects') }}</span>
    </x-slot>
                ...
                <div class="w-full sm:w-1/2 xl:w-1/4 overflow-hidden rounded p-2">
                    <div class="m-2 bg-white shadow border">
                        <img alt="avatar" class="w-full h-56 object-cover object-center"
                             src="{{ $project->getFirstMediaUrl('projects', '430x250') }}">
                        <div class="flex flex-wrap items-center justify-between px-3 py-1 bg-gray-900">
                            <x:a class="rounded px-2 py-1 m-1 text-xs border-b-4 border-l-2 shadow-lg bg-blue-700 border-blue-800" :href="route('admin.projects.edit', $project)">
                        ...
                        <div class="py-4 px-6">
                            <h1 class="text-md font-semibold text-gray-800">
                                <a href="{{ route('projects.show', $project) }}" target="_blank" class="hover:underline">{{ $project->title }}</a>
                            </h1>
                            <div class="text-sm">{!! $project->excerpt !!}</div>
                        </div>
                    </div>
                </div>
            @endforeach
